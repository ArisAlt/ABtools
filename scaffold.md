...
- `ablib.metadata.utils` owns the resolvers both organisers share: `parse_book_folder_name` (reads `<Author> - <Series> - Book <N> - <Title>`), `is_plausible_author` (rejects disc markers, track indices and filename echoes), `normalise_author` and `primary_author`. `ablib.providers.http` cleans the query (`clean_query_title`), scores each dimension only when there is something to compare against (`score_candidate`), runs a ladder of progressively looser queries, lifts inline series out of catalogue titles (`split_series_suffix`), caches per query and disables Goodreads once it throttles. `ablib.providers.mcp` holds the MCP tool definitions. The MCP prompt enforces running Goodreads before Audible and pulls DuckDuckGo snippets when needed.
